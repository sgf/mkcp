...
namespace mkcp {
    ...
    public static class SocketHelper {
        private static Socket GetShareConfigSocket(ProtocolType protocol = ProtocolType.Tcp) {
            Socket _socket = null;
        ...
        public static Socket GetClientSocket() {
            var _socket = GetShareConfigSocket();
            _socket.SendBufferSize = _16K;
            _socket.ReceiveBufferSize = _16K;
            return _socket;
        }
        ...
        /// </summary>
        /// <returns></returns>
        public static Socket GetTransferClientSocket() {
            var _socket = GetShareConfigSocket();
            _socket.SendBufferSize = _64K;
            _socket.ReceiveBufferSize = _64K;
            return _socket;
        }
        ...
        public static Socket GetSessionSocket() {
            var _socket = GetShareConfigSocket();
            _socket.SendBufferSize = _16K;//考虑到 MMO 一般来说服务器发送的数据量要比客户端大因此 Session的发送缓冲区 设稍大一点
            _socket.ReceiveBufferSize = _8K;
            return _socket;
        }

        public static Socket GetServerAcceptSocket() {
            var _socket = GetShareConfigSocket();
            _socket.ReceiveBufferSize = 0;//Accept Socket 因为无须进行任何的数据收发 因此缓冲区置零
            _socket.SendBufferSize = 0;//设置Socket底层SO_SNDBUF为0 等同于：SetSocketOption(SocketOptionLevel.Socket, SocketOptionName.SendBuffer, 0);
        ...
        public static Socket GetUdpSvrSocket(IPEndPoint iPEndPoint) {
            var _socket = GetShareConfigSocket(ProtocolType.Udp);
            //收发缓冲区大小将直接影响丢包等(Udp缓冲区满会丢包)
            _socket.SendBufferSize = _8mb;
            _socket.ReceiveBufferSize = _8mb;
        ...
        public static Socket GetUdpClientSocket() {
            //收发缓冲区大小将直接影响丢包等(Udp缓冲区满会丢包)
            var _socket = GetShareConfigSocket(ProtocolType.Udp);
            _socket.SendBufferSize = _32k;
            _socket.ReceiveBufferSize = _32k;
